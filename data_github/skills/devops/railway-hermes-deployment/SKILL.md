...
## PUSH CODE DIRECTLY FROM RAILWAY SERVER (no local machine needed)

The agent can push to GitHub directly from the Railway container using a GitHub PAT. This is useful when you want to commit updated skills, memories, or bot.py without downloading files to a local machine.

**Confirmed repo:** `github.com/Rajeevboy/claude-testing` (from `RAILWAY_GIT_REPO_NAME` env var)

### Steps:
```bash
# 1. Clone the repo using the PAT
cd /tmp
git clone https://ghp_YOURTOKEN@github.com/Rajeevboy/claude-testing.git

# 2. Configure git identity
cd claude-testing
git config user.email "bot@tention.ai"
git config user.name "Tention Bot"

# 3. Copy files from volume into the repo
cp /app/data/skills/... skills/...
cp /app/data/memories/MEMORY.md data_github/memories/
cp /app/data/memories/USER.md data_github/memories/
...
2. Scope: `repo` only
3. Expiration: 7 days (revoke after use)
4. Ask Rajeev to paste it in chat — use it once, push, done

### Important:
- The PAT goes in the clone URL — never commit it to a file
- Rajeev should revoke the token at github.com/settings/tokens after use
- The Railway env var `RAILWAY_GIT_REPO_OWNER=Rajeevboy` and `RAILWAY_GIT_REPO_NAME=claude-testing` tell you the exact repo without guessing

## PITFALLS

- **Always discover the repo name from env vars, never guess** — `RAILWAY_GIT_REPO_OWNER` and `RAILWAY_GIT_REPO_NAME` are always set. Run `env | grep -i railway` to confirm before cloning. For Rajeev: owner=`Rajeevboy`, repo=`claude-testing`.
- **rsync is not available on Railway** — use `find + while read + cp` pattern instead. See the export commands in the GITHUB-PORTABLE DEPLOYMENT section.
...
- **.gitignore `memories/` pattern blocks `data_github/memories/`** — the existing repo .gitignore has a broad `memories/` rule that silently ignores `data_github/memories/MEMORY.md` and `USER.md`. Git will stage them but they won't actually commit. Fix by adding these exception lines AFTER the `memories/` line in .gitignore:
  ```
  # Exception — data_github/memories is intentionally committed for portable deploy
  !data_github/memories/
  !data_github/memories/*.md
  ```
  Verify with `git check-ignore -v data_github/memories/MEMORY.md` — if it outputs a match, the exception isn't working yet. Git negation rules require the exception to appear AFTER the blocking pattern.
