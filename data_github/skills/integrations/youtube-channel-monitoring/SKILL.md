...
| Vaibhav Sisinty (AI tools / productivity) | @vaibhavsisinty | `UClXAalunTPaX1YV185DWUeg` |

> ✅ `UClXAalunTPaX1YV185DWUeg` confirmed working July 2026. Full channel DB at `/app/data/vaibhav_sisinty_channel_db.json` (105 long videos, 541 Shorts, 646 total — scraped 2026-07-03). Re-scrape to refresh.
| Nate Herk (AI Automation / n8n) | @nateherk | `UC2ojq-nuP8ceeHqiroeKhBA` | June 2026 |
| Vaibhav Sisinty (AI tools / productivity) | @vaibhavsisinty | `UClXAalunTPaX1YV185DWUeg` | July 2026 |
...
## 📋 Rajeev's Required Summary Format — Mind-Map Style (NOT Tables/FAQs)

When Rajeev asks for video summaries to go into a doc, use THIS exact structure:

```
# [Video Title]

## 🤖 [Main Topic Heading — emoji + topic]
- Bullet point about main point
- Bullet point about result/outcome
- Bullet point about why it matters
...
**Rules:**
- Use `#` for video title, `##` for main topic (with emoji), `###` for sub-sections
- ALL bullet points — no tables, no FAQs, no "Key Insights" headers, no "Core Concepts" blocks
- Mind-map tree structure: main topic → sub-sections → bullets
- Emoji on `##` headings only
- Keep it tight — 3-5 bullet points per section, 2-4 sub-sections per video
- Each video entry also needs: upload date + YouTube link above the `#` title

**Example Rajeev approved:**
```
📅 [Date] | 🔗 https://youtube.com/shorts/VIDEO_ID

# Using AI for Job Applications: A Practical Guide

## 🤖 AI-powered Job Applications
- The user applied for 100 jobs in a single day using an AI tool.
- Resulted in getting four genuine interview invitations.

### Tools and Setup
- Use Claude AI platform.
- Navigate to 'connectors' section and install the 'Appify' connector.

### Job Application Process
- Drop your resume into the chat interface.
- Paste a specific prompt and run it.
```

> ✅ Rajeev confirmed this format July 2026. He previously rejected a table-based format with "Key Insights / Core Concepts / Potential FAQs" sections — do NOT use that structure.
...
When Rajeev asks for a "summary doc" of channel videos:
- **Default output: Google Doc** (saved to Drive, shareable link) — confirmed preference July 2026
- NOT Excel, NOT .docx, unless explicitly asked
- Use the "daily-post-linkedin" Drive folder structure or a new named folder
- Share as anyone-reader link
...
Historical snapshots for tracked channels are in `references/channel-snapshots.md` — use these as baselines when Rajeev asks about growth or when deciding whether to re-scrape. Update the snapshot entry after each fresh scrape.

---

## ⚠️ Transcript/Caption Fetching is FULLY BLOCKED on Railway

YouTube aggressively blocks Railway server IPs from all caption/transcript access. **Every method below was tested and confirmed blocked (July 2026):**

| Method | Result |
|--------|--------|
| `youtube-transcript-api` `.fetch()` | `IpBlocked` error |
| `yt-dlp --write-auto-sub` | 429 + "Sign in to confirm you're not a bot" |
| Direct `timedtext` API (`/api/timedtext?v=...`) | Returns empty (0 bytes) |
| YouTube player API (`/youtubei/v1/player`) — WEB/IOS/TVHTML5/WEB_EMBEDDED_PLAYER clients | 0 caption tracks returned |
| YouTube-nocookie embed page | No captionTracks in HTML |
| Invidious public instances | All shut down, 401, or 502 |
| Piped public instances | All shut down or 502 |
| `youtubetranscript.com` | "YouTube is currently blocking us" |
| `supadata.ai` transcript API | 404 |

**The ONLY working solutions:**
1. **User's browser cookies** — export via "Get cookies.txt LOCALLY" Chrome extension → save to `/tmp/cookies.txt` → pass to yt-dlp: `yt-dlp --cookies /tmp/cookies.txt --write-auto-sub ...`
   → See `references/transcript-unblock-cookies-workflow.md` for full step-by-step including batch processing 500+ Shorts.
2. **Paid proxy/residential IP** — route yt-dlp through a residential proxy

**When transcripts are unavailable:** Fall back to AI-generated structured mind-map summaries from video title + description. For channels like Vaibhav Sisinty whose titles are highly descriptive, this produces solid quality summaries.
...
- **`youtube-transcript-api` API changed** — `.get_transcript()` no longer exists. New method: `api = YouTubeTranscriptApi(); t = api.fetch(video_id)`. Moot on Railway anyway (IpBlocked error always).
- **ALL transcript methods blocked on Railway (confirmed July 2026)** — IOS/WEB/TVHTML5/WEB_EMBEDDED_PLAYER player clients all return 0 caption tracks. Direct timedtext API returns empty. All Invidious/Piped instances are down. Don't try multiple clients — they all fail. Only fix: user browser cookies passed via `yt-dlp --cookies /tmp/cookies.txt`.
- **JSON file storage on Railway is ephemeral at `/root/`** — scraped data saved to `/root/` between sessions will be gone after Railway restarts. **Always save channel DBs to `/app/data/<channel_slug>_channel_db.json`** — that path is on the Railway persistent volume and survives restarts. Check if the file already exists before re-scraping; if it does and the data is less than a week old, load it rather than re-scraping. If re-analysis is needed, scrape again fresh.
